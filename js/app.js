...
nction autoFetchWeather() {
  ...
  navigator.geolocation.getCurrentPosition(
    async (pos) => {
      ...
      btn.textContent = '📡 正在获取天气...';
      status.textContent = '已获取位置 (' + lat + ', ' + lon + ')';
      status.style.display = 'block';
      
      try {
        const url = 'https://api.open-meteo.com/v1/forecast' +
          '?latitude=' + lat + '&longitude=' + lon +
          '&current=temperature_2m,pressure_msl,weather_code,wind_speed_10m' +
          '&timezone=auto';
        ...
        const weatherDesc = wmoMap[code] || '多云';
        ...
        document.getElementById('weather-pressure').value = pressure;
        document.getElementById('weather-temp').value = temp;
        document.getElementById('weather-wind').value = windLevel;
        document.getElementById('weather-type').value = weatherDesc;
        
        status.textContent = '\u2705 已获取: ' + temp + '\u2103 ' + pressure + 'hPa ' + windKmh + 'km/h ' + weatherDesc;
